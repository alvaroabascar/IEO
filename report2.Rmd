...
Functional annotations are descriptions of biological activity assigned to structural elements of the genome (e.g., DNA sites, genes, etc.) and to features from high-throughput assay platforms (e.g., microarrays). Functional annotations range from molecular mechanisms (e.g., DNA binding) to high-level phenotypes (e.g., Mendelian genetic disorders).

Usually, functional annotations are partial, noisy, dynamic and their coverage varies drastically among organisms. However, they are essential to any downstream bioinformatic analysis of high-throughput assay data.
...
In order to do this we performa a _Gene Ontology Analysis_:
...
We start searching for DE genes at 10% FDR between recurrent and non-recurrent samples from the results obtained using Limma in the previous step:
...
params <- new("GOHyperGParams", geneIds = DEgenes, universeGeneIds = featureNames(filtered_eset), annotation = "org.Hs.eg.db", ontology = "BP", pvalueCutoff = 0.05, testDirection = "over")

...
We can estimate the surrogate values:

```{r}
sv = sva(exprs(eset), mod, mod0)
```

Finally we can examine whether any of the surrogate variables that we obtained in the previous block relates with any possible source of non-biological variability:
```{r}
...
```{r}
# Control patients have no survival time, so we must remove them to proceed with the analysis
...
In this case we see how all genes are clustering around the diagonal, and we cannot appreciate any one specially deviated. This suggest that we don't have any gene with an obvious differential expression. In the right graph we can see how some genes deviate from the bulk, but maybe not enough to consider them as differentially expressed. However, to verify this, we can perform some additional analysis.
...
```
In the volcano plot we should see a red

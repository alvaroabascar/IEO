...
```XXX{r chip_images, out.width="800px", dpi=400}
...
```XXX{r intensity_boxplots, out.width="800px"}
...
````XXX{r density_plots, out.width="800px", dpi=300}
# Density plot
par(mar = c(3, 3, 4, 1))
...
````XXX{r intensities, out.width="800px"}
...
if (sum(tt$adj.P.Val < 0.05) > 0) {
  abline(h=-log10(max(tt[tt$adj.P.Val < 0.05, "P.Value"])), col=grey(0.5), lty=2)
}
points(tt[tt$adj.P.Val < 0.05, "logFC"], -log10(tt[tt$adj.P.Val < 0.05, "P.Value"]), pch=".", cex=4, col="red")
```
...
if (sum(ttadj$adj.P.Val < 0.05) > 0) {
  abline(h=-log10(max(ttadj[ttadj$adj.P.Val < 0.05, "P.Value"])), col=grey(0.5), lty=2)
}
points(ttadj[ttadj$adj.P.Val < 0.05, "logFC"], -log10(ttadj[ttadj$adj.P.Val < 0.05, "P.Value"]), pch=".", cex=4, col="red")
```
...
DEgenes <- rownames(ttadj[ttadj$adj.P.Val < 0.05, , drop = FALSE])
length(DEgenes)
...
params <- new("GOHyperGParams", geneIds = DEgenes_indb$ENTREZID, universeGeneIds = universeIDs$ENTREZID, annotation = "hgu133plus2.db", ontology = "BP", pvalueCutoff = 0.05, testDirection = "over")
...
```{r}
# Select gene ID in database, in this case the gene universe is restricted to the 
# subeset of recurrents:
DEgenes_indb <- select(hgu133plus2.db, recurrent_overexpressed, "ENTREZID", "PROBEID")
universeIDs <- select(hgu133plus2.db, featureNames(eset_recurrent), "ENTREZID", "PROBEID")
...
universeIDs <- select(hgu133plus2.db, featureNames(eset_norecurrent), "ENTREZID", "PROBEID")
...
The main idea of this part is to calculate for each gene set an enrichment score (ES) as function of the changes in gene expression by the genes forming the gene set.

Firstly, we generate the vector of genes based on the results of Gene Ontology:

```{r}
allHumanGO <- select(hgu133plus2.db, columns = "PROBEID", key ="GO:1902106", keytype = "GO")
...
#gsc <- getGmt("c5.bp.v5.0.orig.gmt", geneIdType=SymbolIdentifier(), collectionType=GOCollection(ontology="BP"))
...
We can evaluate the Z-scores to detect differential expressions within genesets, using the standardized sample mean of the geneset t-statistics. To add some robustness to the calculation of the Z-scores, we limit the evaluation to genesets having more than 10. We store all moderated t-statistics for the genes forming the incidence matrix of gene sets and calculate its Z-score statistic.
...
For the GO terms NERVOUS_SYSTEM_DEVELOPMENT, SKELETAL_DEVELOPMENT and IMMUNE_RESPONSE, we can observe the mean expression values comparing the two phenotypes.
...
genesGS1 <- colnames(Im)[which(Im["NERVOUS_SYSTEM_DEVELOPMENT", ] == 1)]
genesGS2 <- colnames(Im)[which(Im["SKELETAL_DEVELOPMENT", ] == 1)]
genesGS3 <- colnames(Im)[which(Im["IMMUNE_RESPONSE", ] == 1)]
par(mfrow = c(1, 3), mar = c(4, 5, 3, 4))
plotGS(eset_GSEA, genesGS1, "disease_status", main = "NERVOUS_SYSTEM_DEVELOPMENT", cex.lab = 2, las = 1)
plotGS(eset_GSEA, genesGS2, "disease_status", main = "SKELETAL_DEVELOPMENT", cex.lab = 2, las = 1)
plotGS(eset_GSEA, genesGS3, "disease_status", main = "IMMUNE_RESPONSE", cex.lab = 2, las = 1)
```

To calculate the adjusted p-values associated to each GO term, by and select those with FDR < 5%.

```{r}
pv <- pmin(pnorm(zS), 1 - pnorm(zS))
sum(pv < 0.05)
pvadj <- p.adjust(pv, method = "fdr")
DEgs <- names(pvadj)[which(pvadj < 0.05)]
length(DEgs)
head(DEgs, n = 3)
```
